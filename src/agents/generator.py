...
def _generate_step_code(step: Dict[str, Any]) -> str:
    ...
    if action == "fill":
        return ActionTemplate.FILL.format(selector=step["selector"], value=step["value"])
    if action == "click":
    ...
    if action == "assert_text":
        return ActionTemplate.ASSERT_TEXT.format(
            selector=step["selector"], contains=step.get("contains", "")
        )
...
def generate_test_code(plan: Dict[str, Any]) -> str:
    ...
    for step in plan["steps"]:
        try:
            ...
            raise ValueError(f"Missing required field in step: {e}. Step: {step}") from e

    body = "\n".join(steps_code)

    test_code = TEST_FUNCTION_TEMPLATE.format(test_name=plan["test_name"], body=body)
...
def generate_multiple_tests(plans: List[Dict[str, Any]], output_dir: str) -> List[str]:
    ...
    for plan in plans:
        ...
        body = "\n".join(steps_code)
        test_func = TEST_FUNCTION_TEMPLATE.format(test_name=plan["test_name"], body=body)
