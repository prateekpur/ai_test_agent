...
def main():
    ...
    parser.add_argument(
        "-i", "--input", required=True, help="Input file containing test descriptions"
    )

    parser.add_argument(
        "-o", "--output", required=True, help="Output file for generated pytest tests"
    )
    ...
    parser.add_argument(
        "--api-base", help="Custom API base URL (default: OpenAI or GITHUB_TOKEN endpoint)"
    )

    args = parser.parse_args()

    try:
        llm_client = LLMClient(api_base=args.api_base, model=args.model)
